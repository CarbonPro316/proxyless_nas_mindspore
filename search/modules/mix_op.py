import numpy as np
from mindspore.Parameter import Parameter
...
class MixedEdge(MyModule):
    ...
    def __init__(self, candidate_ops):
        ...
        self.candidate_ops = nn.CellList(candidate_ops)
        self.AP_path_alpha = Parameter(mindspore.Tensor(self.n_choices))  # architecture parameters
        self.AP_path_wb = Parameter(mindspore.Tensor(self.n_choices))  # binary gates
    ...
    def probs_over_ops(self):
        probs = O.Softmax(self.AP_path_alpha)  # softmax to probability
        return probs
    ...
    def entropy(self, eps=1e-8):
        ...
        log_probs = O.Log(probs + eps)
        op = O.ReduceSum()
        mul = O.Mul()
        entropy = - op(mul(probs, log_probs))
        return entropy
        ...
        if MixedEdge.MODE == 'full' or MixedEdge.MODE == 'two':
            ...
            def backward_function(candidate_ops, active_id, binary_gates):
                def backward(_x, _output, grad_output):
                    zeroslike = O.ZerosLike()
                    binary_grads = zeroslike(binary_gates.data)
                    with 1:
    ...
    def binarize(self):
        ...
        if MixedEdge.MODE == 'two':
            ...
            arr[max1] = 0
            max2 = np.argmax(arr)
            out = np.array([max1, max2])
            sample_op = mindspore.Tensor(out)
            probs_slice = O.Softmax(O.Stack([
            ...
            arr = probs.asnumpy(probs_slice.data)
            max1 = np.argmax(arr)
            out = np.array([max1])
            c = mindspore.Tensor(out)
            c = c[0]
            ...
            self.log_prob = O.Log(probs[sample])
    ...
    def set_arch_param_grad(self):
        ...
        if self.AP_path_alpha.grad is None:
            zeroslike = O.ZerosLike()
            self.AP_path_alpha.grad = zeroslike(self.AP_path_alpha.data)
        if MixedEdge.MODE == 'two':
