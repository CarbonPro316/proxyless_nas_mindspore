...
class ArchSearchConfig:
    ...
    def build_optimizer(self, params):
        ...
        if self.opt_type == 'adam':
            return mindspore.nn.Adam(
                params, learning_rate=self.lr, weight_decay=self.weight_decay, **self.opt_param
            )
        else:
            raise NotImplementedError
...
class ArchSearchRunManager:
    ...
    def gradient_step(self):
        ...
        if self.arch_search_config.target_hardware is None:
            ...
            input_var = zeros(data_shape, device=self.run_manager.device)
